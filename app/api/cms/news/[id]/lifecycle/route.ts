...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (action === 'trash') moveCmsArticleToTrash(id, admin.id)
    else if (action === 'restore') restoreCmsArticle(id, admin.id)
    else if (action === 'delete') deleteCmsArticlePermanently(id, admin.id)
    else if (action === 'pin') setCmsArticlePinned(id, true, admin.id)
    else if (action === 'unpin') setCmsArticlePinned(id, false, admin.id)
    else if (action === 'reorder') reorderCmsPublishedArticles(ids ?? [], admin.id)
