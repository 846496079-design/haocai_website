...
export default async function CNNewsArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const article = getPublishedArticle('cn', slug)
  if (!article) notFound()
  return <OfficialSite site={getSiteContent('cn')} page="newsDetail" articleSlug={slug} initialArticles={getPublishedArticles('cn')} initialArticle={article} />
}
