...
export default function CmsNewsEditor({ initial }: { initial: CmsArticleRecord }) {
  ...
  async function uploadImage(file: File) {
    ...
    setSaving(false)
    if (!response.ok || !data.url) { setNotice(data.message ?? '图片上传失败。'); return undefined }
    return data.url
  }
  ...
  async function uploadCover(file: File) {
    ...
    if (!url) return
    setContent((current) => Object.fromEntries(CMS_LOCALES.map((code) => [code, { ...current[code], cover: url }])) as CmsArticleContent)
  ...
  async function translate() {
    ...
    setSaving(true)
    const response = await fetch(`/api/cms/news/${initial.id}/translate`, { method: 'POST' })
    const data = await response.json() as { message?: string }
    setSaving(false)
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-6 text-slate-900 md:px-8"><div className="mx-auto max-w-6xl"><header className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 pb-5"><div><Link href="/cms/" className="text-sm font-semibold text-indigo-600">返回新闻管理</Link><h1 className="mt-2 text-2xl font-bold">编辑草稿：{initial.slug}</h1><p className="mt-1 text-sm text-slate-500">状态：{initial.status === 'PUBLISHED' ? '已发布，当前编辑将作为新草稿保存' : '草稿'}。发布前必须完成三语预览与人工确认。</p></div><div className="flex flex-wrap gap-2"><button onClick={saveDraft} disabled={saving} className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold disabled:opacity-60">{saving ? '处理中…' : '保存草稿'}</button><button onClick={preview} disabled={saving} className="rounded-lg bg-slate-950 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60">预览官网效果</button></div></header>
    <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_280px]"><section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-wrap gap-2 border-b border-slate-200 pb-4">{CMS_LOCALES.map((code) => <button key={code} onClick={() => setLocale(code)} className={`rounded-lg px-3 py-2 text-sm font-semibold ${locale === code ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600'}`}>{localeNames[code]}</button>)}</div>
      <div className="mt-6 space-y-5"><div className="grid gap-4 sm:grid-cols-2"><Field label="发布日期"><input type="date" value={article.date.replaceAll('.', '-')} onChange={(event) => updateArticle({ date: event.target.value.replaceAll('-', '.') })} /></Field><Field label="分类"><select value={article.category} onChange={(event) => updateArticle({ category: event.target.value })}><option value="">请选择分类</option>{categories.map((name) => <option key={name} value={name}>{name}</option>)}</select><p className="mt-2 text-xs font-normal text-slate-500">没有合适分类时，请先在分类管理中创建；从工作台导入的新分类会自动建立并标为“导入”。</p></Field></div><Field label="新闻标题"><input value={article.title} onChange={(event) => updateArticle({ title: event.target.value })} /></Field><Field label="摘要"><textarea value={article.summary} onChange={(event) => updateArticle({ summary: event.target.value })} rows={3} /></Field><Field label="导语"><textarea value={article.lead} onChange={(event) => updateArticle({ lead: event.target.value })} rows={4} /></Field><Field label="标签（用英文逗号分隔）"><input value={(article.tags ?? []).join(', ')} onChange={(event) => updateArticle({ tags: event.target.value.split(',').map((tag) => tag.trim()).filter(Boolean) })} /></Field>
        ...
        <div className="border-t border-slate-200 pt-5"><div className="flex items-center justify-between gap-3"><div><h2 className="text-lg font-semibold">正文内容块</h2><p className="mt-1 text-sm text-slate-500">拖拽分节可调整正文顺序；每个分节可添加图片。</p></div><button onClick={() => updateArticle({ sections: [...article.sections, { title: '', paragraphs: [''] }] })} className="rounded-lg border border-indigo-200 px-3 py-2 text-sm font-semibold text-indigo-700">新增分节</button></div><div className="mt-4 space-y-5">{article.sections.map((section, index) => <div key={index} draggable onDragStart={() => setDraggingIndex(index)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (draggingIndex !== null) moveSection(draggingIndex, index); setDraggingIndex(null) }} className={`rounded-xl border p-4 ${draggingIndex === index ? 'border-indigo-400 bg-indigo-50' : 'border-slate-200'}`}><div className="flex items-center justify-between gap-3"><span className="cursor-grab text-sm font-semibold text-slate-500">拖动排序 · 分节 {index + 1}</span><button onClick={() => updateArticle({ sections: article.sections.filter((_, sectionIndex) => sectionIndex !== index) })} className="text-sm text-red-600">删除</button></div><Field label="分节标题"><input value={section.title} onChange={(event) => updateSection(index, { title: event.target.value })} /></Field><Field label="正文段落（段落之间空一行）"><textarea value={section.paragraphs.join('\n\n')} onChange={(event) => updateSection(index, { paragraphs: event.target.value.split(/\n\s*\n/).map((paragraph) => paragraph.trim()).filter(Boolean) })} rows={8} /></Field><Field label="分节图片"><div className="flex flex-col gap-2"><input value={section.image ?? ''} onChange={(event) => updateSection(index, { image: event.target.value })} placeholder="图片地址" /><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadSectionImage(index, file) }} />{section.image && <img src={section.image} alt="正文图片预览" className="max-h-56 rounded-lg border border-slate-200 object-cover" />}</div></Field></div>)}</div></div>
      ...
      </div></section><aside className="h-fit rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="font-semibold">发布检查</h2><ul className="mt-4 space-y-3 text-sm text-slate-600"><li>三语内容：{complete ? <b className="text-emerald-700">完整</b> : <b className="text-amber-700">待补全</b>}</li><li>译稿状态：{initial.translationStatus === 'CURRENT' ? <b className="text-emerald-700">已基于当前中文生成</b> : initial.translationStatus === 'STALE' ? <b className="text-amber-700">中文已修改，请重新翻译</b> : <b className="text-amber-700">尚未生成译稿</b>}</li><li>草稿预览：{initial.previewedAt ? '已预览过当前版本' : '保存后点击预览'}</li><li>正式发布后，草稿将替换当前线上版本。</li></ul><button onClick={translate} disabled={saving} className="mt-5 w-full rounded-lg border border-indigo-200 px-4 py-3 text-sm font-semibold text-indigo-700 disabled:opacity-50">保存并一键翻译日文、繁体</button><p className="mt-2 text-xs text-slate-500">翻译会基于当前中文草稿执行一次，完成后仍需人工审核。</p><label className="mt-5 flex cursor-pointer gap-2 rounded-lg bg-amber-50 p-3 text-sm text-amber-900"><input type="checkbox" checked={reviewed} onChange={(event) => setReviewed(event.target.checked)} className="mt-1" />我已人工审核三语内容、封面和预览效果，确认正式发布。</label><button onClick={publish} disabled={saving || !complete} className="mt-3 w-full rounded-lg bg-indigo-600 px-4 py-3 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50">确认发布</button>{initial.status === 'PUBLISHED' && <button onClick={offline} className="mt-3 w-full rounded-lg border border-red-200 px-4 py-3 text-sm font-semibold text-red-700">下线新闻</button>}{notice && <p className="mt-4 rounded-lg bg-slate-100 p-3 text-sm text-slate-700">{notice}</p>}</aside></div></div></main>
}
