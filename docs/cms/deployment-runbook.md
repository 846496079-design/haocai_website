...
4. 素材：分别上传 JPEG、PNG、WebP；正常情况显示“上传成功”并返回持久 URL。若图片优化服务故障，页面应显示“已保存原图”且仍可继续保存、预览和发布；刷新和重新部署后图片仍可访问。
5. 稿件：新建中文草稿、加入正文图、拖拽分节、保存；日文和港文不应自动变化。
6. 必填引导：简体中文页仅分类、新闻标题、摘要、新闻封面、正文内容显示 `*`。清空摘要和封面后点击“确认发布”，按钮应可点击并一次提示全部缺项、切回中文页、定位第一项；发布日期、作者和标签不得被判为缺项。
7. 非阻塞发布：不翻译，补全中文后按提示保存、预览并人工确认发布；CN 列表和详情可访问，JP/HK 不展示该稿，CMS 标识待翻译。
8. 翻译：点击后编辑区应被处理遮罩锁定；生成 JP、HK 草稿后，页面应明确显示“翻译已完成并保存”并直接引导预览。人工检查产品名、数字、日期和链接，返回编辑页勾选人工审核并发布，再验证三个站点的列表和详情。
9. 状态：下架后公开详情不可访问；删除进入回收站；恢复回到草稿。
10. 已发布排序：置顶和拖拽只出现在已发布区，三个公开站点顺序一致。
11. 外部导入：使用 Preview 密钥导入一次新分类与一次空分类，确认都只生成草稿。
