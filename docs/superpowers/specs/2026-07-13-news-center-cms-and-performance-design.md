...
> 2026-07-20 后续变更：本文原有“三语完整才可发布”门禁已由 `2026-07-20-cms-optional-translation-publish-design.md` 取代。当前规则是中文完整即可发布，未翻译稿只在中文站展示。
...
| 决策 | 采用官网内置轻量 CMS，草稿经人工审核后正式发布 |

## 1. 背景与目标

当前新闻数据写在 `lib/news-content.ts`，官网以纯静态方式导出。新闻页慢感的主要来源是未优化的 PNG 封面（当前新闻资源约 28 MB）和客户端携带的完整文章数据，而不是静态内容本身。

新媒体部门后续需要独立维护新闻，工作流已经明确为“编辑草稿、预览官网效果、人工确认、正式发布”。第一期只有一个独立 CMS 管理员，不复用财税产品的账号、权限、登录入口或业务数据。
...
4. 审核确认后由 CMS 执行正式发布，管理员可明确看到发布成功或失败状态，不再需要人工修改新闻代码。

## 2. 方案比较与结论

### 方案 A：官网内置轻量 CMS（采用）

官网从 `output: 'export'` 迁移为运行在 Node.js 环境的 Next.js 服务。CMS 管理页、预览页、公开新闻页共用同一套新闻展示组件；CMS 通过独立 cookie 会话保护，公开页仅查询已发布版本。发布提交后更新当前公开版本并失效新闻缓存，管理员可看到完整的发布结果。

优点：预览与官网真实效果一致；发布链路可追溯；新闻数据不再进入大客户端包；只维护一个仓库和一套展示组件。当前实现使用可配置路径的 SQLite 以便本地单管理员运行；生产环境需要提供 Node.js 服务、持久化数据库和上传文件存储，不能继续只部署 `dist` 静态目录。
...
可保留纯静态托管，但发布需要等待构建和部署完成；草稿预览也需要额外预览构建，运维与内容发布链路较长。本轮不采用。
...
- 发布用单个数据库事务更新文章当前版本和状态；成功后失效新闻列表、首页新闻摘要、三站详情页缓存，返回可复制的线上链接和明确发布结果。
...
| 领域 | 验收标准 |
| --- | --- |
| 发布结果 | 审核确认后才能提交正式发布；成功时返回三站线上链接，失败时保留草稿和原线上版本，并给出可操作错误信息 |
...
1. **生产 Node.js 服务**：需要提供能够持续运行 Next.js 服务的部署位置和反向代理，纯 `dist` 静态托管不能承载 CMS。
2. **生产数据库与备份**：当前单管理员实现使用 SQLite 文件，可通过 `CMS_DATABASE_PATH` 挂载到持久卷；若部署为多实例或需要统一运维，应迁移至独立 MySQL 数据库或 schema，不得直接与财税业务表混用。
