...
- 本地验收版已完成四个稿件工作区、组合筛选、分类完整增删改查、软删除/恢复/永久删除、发布后置顶和拖拽排序。
- 编辑器已使用日期与分类选择控件，正文分节支持图片上传、替代文字、图片说明与拖拽排序；分类为空时会在发布检查中阻止发布。
- 一键翻译已接入可配置的服务适配层：未配置 API 时返回可执行的配置提示，配置后一次生成日文与繁体草稿，仍需人工审核后发布。
- 已补充账号改密、同源写操作保护、稿件操作记录、导入幂等记录、翻译任务记录和图片资产记录。
- Neon 迁移、旧稿迁移、完整性校验、环境变量及运维步骤见 `docs/cms/`，相关脚本见 `scripts/cms/`。
- 当前 SQLite 与本地图片目录仅用于开发验收。配置 `CMS_DATABASE_URL` 后，CMS 运行时改用 Neon PostgreSQL；配置 `BLOB_READ_WRITE_TOKEN` 后图片改写入 Vercel Blob。部署前仍必须配置翻译服务环境变量并对真实资源完成验收。

## 14. 外部工作台导入接口（已预留）

- 地址：`POST /api/integrations/news/import`。
- 认证：使用 `CMS_IMPORT_SIGNING_SECRET` 对“时间戳、幂等键、原始请求体”进行 HMAC-SHA256 签名，完整请求头与签名示例见 `docs/cms/integration-api.md`。
- 请求体包含 `sourceId` 与三语 `content`；其中 `content.cn.slug` 必填。同一幂等键重复投递返回原结果；重复 slug 只更新草稿，不直接覆盖线上已发布版本。
