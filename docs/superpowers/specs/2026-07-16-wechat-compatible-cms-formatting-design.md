...
图片粘贴行为对标微信公众号编辑工作台：编辑人员不需要先保存剪贴板图片为本地文件，也不需要手工复制图片地址。Markdown 正文区和实时预览区都是图片粘贴入口，粘贴完成后仍统一落为 Markdown 图片语法和 CMS 素材记录。

| 剪贴板来源 | 识别方式 | 处理结果 |
| --- | --- | --- |
| Windows、macOS 截图 | `clipboardData.items` 中的图片文件 | 自动上传并插入当前光标位置 |
| 本地文件复制 | `items` 或 `files` 中的图片文件 | 自动上传并插入，不要求再次选择文件 |
| 网页右键复制图片 | `text/html` 中的 `img` 及其 `src`、`data-src`、`data-original` | 服务端下载、校验、转码并保存为 CMS 素材 |
| Base64 图片 | `data:image/*` | 客户端转为图片文件后走现有上传链路，草稿中不保留 Base64 |
| 图片 URL | `text/uri-list`，或只包含图片 URL 的纯文本剪贴板 | 服务端导入 CMS 素材；普通正文链接仍按文本粘贴 |
| 连续多图 | 同一次剪贴板事件中的多项图片 | 按剪贴板顺序建立占位并逐项替换，异步完成顺序不得漂移 |

粘贴开始时立即在当前选择区建立唯一占位符，上传成功后原位替换为 `![alt](cms_public_url)`；编辑人员在上传期间继续输入时，图片位置不得跳到文末。失败图片只移除自己的占位符并显示具体原因，不清空其他成功图片和原正文。上传未完成的占位符必须阻止保存、复制和发布。

网页图片导入必须满足以下边界：

- 只允许 `http` 和 `https`，禁止 URL 用户名、密码和非标准脚本协议。
- DNS 解析结果必须全部是公网地址；拒绝 localhost、环回、私网、链路本地、保留地址和内部域名。
- 最多跟随 3 次重定向，并对每一跳重新执行协议、主机和 DNS 校验。
- 响应体最多 10MB；读取流时实时计数，不能只信任 `Content-Length`。
- 只接收可由图片解码器确认的 JPEG、PNG、WebP、GIF 或 AVIF，不接收 SVG 和任意 HTML。
- 导入后仍转为 CMS 标准 WebP 主图与缩略图；最终 Markdown 不引用来源站临时地址。
...
| 网页图片指向私网或保留地址 | 拒绝服务端请求，移除该图片占位并提示安全校验失败 |
| 多图部分失败 | 成功图片保持原顺序；失败图片单独提示，不回滚其他图片 |
...
5. 图片文章：截图、本地文件复制、网页右键复制、Base64、图片 URL、连续三图、不同宽高比、替代文字和图片说明。
...
- 图片必须插入粘贴时的光标位置；多图异步上传完成顺序不能改变正文顺序。
