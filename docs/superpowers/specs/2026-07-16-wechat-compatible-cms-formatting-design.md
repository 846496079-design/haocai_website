...
图片粘贴行为以 `mspringjade/wechat-formatter` 的公开实现为行为基线，核对版本为提交 `046336c4d9a992537d1c0e908c95d4bf9660c92f`。该实现把 `onPaste` 绑定到 Markdown 文本框，遍历 `clipboardData.items`，发现第一个 `type` 包含 `image` 的条目后调用 `getAsFile()`，阻止默认粘贴并插入图片标记，然后结束本次遍历。

参考实现使用 `FileReader` 把图片保存为当前页面内存中的 Base64 映射。CMS 需要支持保存、刷新和官网发布，因此只复用它的剪贴板识别与光标插入口径，不复用临时 Base64 存储：图片必须通过现有 CMS 上传接口生成稳定地址，再原位替换为 Markdown 图片语法。

| 输入场景 | 识别方式 | CMS 处理结果 |
| --- | --- | --- |
| Windows、macOS 截图 | `clipboardData.items` 中的 `image/*` 条目 | 取第一个图片文件，自动上传并插入粘贴时的选择区 |
| 本地图片文件复制 | 浏览器把剪贴板暴露为 `image/*` 条目 | 与截图走同一上传链路，不要求再次选择文件 |
| 网页右键“复制图片” | 浏览器把复制结果暴露为 `image/*` 条目 | 与截图走同一上传链路；不解析来源网页 HTML |
| 在线图片地址 | 正文图片区域的 HTTPS 地址输入框 | 继续使用显式“插入在线图片”，不把普通文本 URL 当成图片自动导入 |

粘贴开始时立即在当前选择区建立唯一占位符，上传成功后原位替换为 `![alt](cms_public_url)`；编辑人员在上传期间继续输入时，图片位置不得跳到文末。上传失败时恢复粘贴前被选择的正文，不留下临时地址或占位符。上传未完成的占位符必须阻止保存、复制和发布。

本次修复不扩展参考实现没有提供的行为：不解析仅包含 `text/html` 的 `<img>`，不根据 `text/uri-list` 或纯文本 URL 启动服务端下载，不把实时预览区作为粘贴入口，也不批量处理同一次事件中的多个图片条目。如果浏览器没有把剪贴板内容暴露为 `image/*` 条目，则保持普通文本粘贴行为。
...
| 粘贴期间点击保存 | 阻止保存并提示仍有图片正在上传，不把占位符写入草稿 |
...
5. 图片文章：截图、本地图片文件复制、网页右键“复制图片”、显式 HTTPS 图片地址、不同宽高比、替代文字和图片说明。
...
- 剪贴板中的第一个 `image/*` 条目必须插入粘贴时的选择区，上传期间继续输入不能使图片跳到文末。
- 保存后的 Markdown 和 HTML 不得包含 `blob:`、`data:image`、上传占位符或来源站临时地址。
...
- `mspringjade/wechat-formatter` 用于核对可观察行为和字段口径；其仓库许可证和 README 对商业使用另有约束，本项目不直接复制其核心代码、模板或视觉资产，任何超出行为参考的复用须先完成授权审查。
...
| 上传占位符回归 | `npm run verify:publication-formatting` | 通过；未完成图片上传会同时阻止服务端保存和一键复制 |
| 生产构建 | `npm run build` | 通过 |
| 隔离数据库端到端 | 临时 SQLite：登录、建稿、三语保存、快照、哈希、预览、发布、官网读取 | 通过；三语无乱码，脚本标签被移除，官网返回 200 |
| 旧文章兼容 | 临时数据库中的既有已发布文章 | 通过；仍使用旧分节渲染，没有被自动切换到新格式 |
| 图片直接粘贴 | 真实 Chrome 或微信公众号工作台 | 待人工验收；当前内置浏览器自动化不支持正常二进制文件上传，二进制剪贴板桥接结果不能代替真实系统剪贴板 |

真实 CMS 图片直接粘贴、微信公众号编辑器和手机端的粘贴效果仍是上线前人工验收项。该项需要业务人员在真实浏览器和公众号后台执行，结果应按第 13 节记录；本地回归通过不等于浏览器剪贴板和微信侧验收完成。
