...
export async function updateCmsDraft(id: number, draft: CmsArticleContent, adminId: number) {
  ...
  await sql(`UPDATE news_article SET category_id = $1, translation_status = CASE WHEN $2 THEN 'STALE' ELSE translation_status END, status = CASE WHEN published_version_id IS NULL THEN 'DRAFT' ELSE status END, updated_at = NOW() WHERE id = $3`, [categoryId, chineseChanged, id]); await writeAudit(adminId, 'SAVE_DRAFT', 'news_article', String(id), { versionId }); return getCmsArticle(id)
}
