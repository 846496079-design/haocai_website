...
async function main() {
  ...
  const stats = store.getLeadStats()
  assert.equal(stats.total, 2)
  assert.equal(stats.trial, 1)
  assert.equal(stats.partner, 1)
  assert.equal(stats.overdue, 1)
  assert.equal(store.listLeadSubmissions({ kind: 'TRIAL' }).length, 1)
  assert.equal(store.listLeadSubmissions({ kind: 'PARTNER' }).length, 1)
