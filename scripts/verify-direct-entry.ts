...
assert.match(officialSite, /window\.history\.replaceState/)
assert.match(officialSite, /window\.location\.pathname.*window\.location\.search.*formHash/s)
assert.match(officialSite, /formSection\.scrollIntoView\(\{ behavior: 'auto', block: 'start' \}\)/)
assert.match(officialSite, /window\.cancelAnimationFrame\(frameId\)/)
assert.match(siteSpec, /\/cn\/partners\/\?c=LFFWTL/)
assert.match(siteSpec, /\/cn\/partners\/\?c=LFFWTL.*#partner-form/)
assert.match(siteSpec, /\/cn\/\?c=LFFWTL.*不跳转.*不预填/)
